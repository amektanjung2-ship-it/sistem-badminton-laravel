<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Data Alat</h2></x-slot>
    <div class="py-12"><div class="max-w-3xl mx-auto sm:px-6 lg:px-8"><div class="bg-white p-6 shadow-sm sm:rounded-lg">
        ...
        <form action="{{ route('admin.alat.update', $alat->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="number" name="stok" value="{{ $alat->stok }}" class="w-full border-gray-300 rounded-md" required>
            </div>
            <div class="flex justify-end pt-4 border-t">
                <a href="{{ route('admin.alat.index') }}" class="bg-gray-500 text-white py-2 px-4 rounded mr-2">Batal</a>
                <button type="submit" class="bg-blue-600 text-white py-2 px-6 rounded">Simpan Perubahan</button>
            </div>
        </form>
    </div></div></div>
</x-app-layout>